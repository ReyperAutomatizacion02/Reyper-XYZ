...
const ImagePreviewWithZoom = ({ src, fileId }: { src: string; fileId?: string }) => {
    ...
    if (fileId) {
        return (
            <div className="relative h-full w-full overflow-hidden rounded-lg bg-card shadow-2xl">
                <iframe
    ...
    if (imgError) {
        return (
            <div className="flex h-full flex-col items-center justify-center p-4 text-center text-muted-foreground">
                ...
                <p className="mt-2 text-xs opacity-70">Es posible que el archivo sea privado o no compatible.</p>
            </div>
        );
    }
...
const AutoResizeTextarea = ({ value, onChange, placeholder, className, minHeight = "38px" }: any) => {
    return (
        <div
            className={cn(
                "relative grid w-max min-w-full grid-cols-[1fr]",
                className?.includes("max-w") ? "" : "max-w-full"
            )}
        >
            {/* Shadow element for sizing: Matches font/padding of textarea exactly */}
            <div
                className={cn(className, "invisible h-auto w-full overflow-hidden whitespace-pre-wrap break-words")}
                aria-hidden="true"
                style={{ minHeight }}
            >
                {value || placeholder || " "}
            ...
            <textarea
                ...
                className={cn(
                    className,
                    "leading-inherit absolute inset-0 h-full w-full resize-none overflow-hidden outline-none focus:outline-none"
                )}
            />
        </div>
    );
...
export function ProjectForm({
    ...
    const [treatmentsList, setTreatmentsList] = useState(
        treatments ? treatments.map((t) => ({ value: t.id, label: t.name })) : []
    );
    ...
    const filteredContacts = selectedClient ? allContacts.filter((c) => c.client_id === selectedClient) : [];
            ...
            setGeneratedItems([
                {
                    id: 1,
                    stableId: "demo-1",
                    code: "PRJ-260204-01.00",
                    description: "Parte Superior Estructura",
                    quantity: 2,
                    unit: "PZA",
                    design_no: "D-001",
                    material: "Acero A36",
                ...
                },
                {
                    id: 2,
                    stableId: "demo-2",
                    code: "PRJ-260204-02.00",
                    description: "Buje de Bronce",
                    quantity: 10,
                    unit: "PZA",
                    design_no: "D-002",
                    material: "Bronce",
            ...
            [
                {
                    ...
                    popover: {
                        ...
                        description:
                            "Primero selecciona el Cliente, el Usuario solicitante y las fechas de Solicitud y Entrega.",
                        side: "bottom",
                        align: "start",
                    },
                },
                ...
                },
                {
                    ...
                    popover: {
                        ...
                        description:
                            "Aquí aparecen los archivos cargados. Puedes elegir cuáles incluir en el proyecto final.",
                        side: "top",
                        align: "center",
                    },
                },
                {
                    ...
                    popover: {
                        ...
                        description:
                            "Arrastra las filas desde el icono de la izquierda para cambiar su orden en el proyecto.",
                        side: "right",
                        align: "center",
                    },
                },
                ...
                },
                {
                    ...
                    popover: {
                        ...
                        description:
                            "Activa este switch para convertir una partida en subpartida, vinculándola a la anterior.",
                        side: "left",
                        align: "center",
                    },
                },
                {
                    ...
                    popover: {
                        ...
                        description:
                            "Una vez todo esté correcto, haz clic aquí para registrar el proyecto y sus partidas en el sistema.",
                        side: "top",
                        align: "end",
                    },
                },
                ...
                if (quoteClient) {
                    ...
                    try {
                        ...
                        if (initialQuote.items && initialQuote.items.length > 0) {
                            const quoteItemsAsGenerated: GeneratedItem[] = initialQuote.items.map(
                                (item: any, idx: number) => {
                                    return {
                                        id: idx + 1,
    ...
    const handleGeneratePreview = async () => {
        ...
        try {
            ...
            const nextCode =
                fetchedCode || projectCode || `${clientCode || "PRJ"}-${format(requestDate, "yyMMdd")}-XXXX`;
    ...
    const onFilesSelected = async (files: File[]) => {
        ...
        setLoading(true);
        try {
            const newStaging: StagingFile[] = await Promise.all(
    ...
    const updateItem = (id: number, field: keyof GeneratedItem, value: any) => {
        setGeneratedItems((prev) => prev.map((item) => (item.id === id ? { ...item, [field]: value } : item)));
    };
    ...
    const ghostInputClass =
        "bg-transparent border-transparent shadow-none hover:bg-primary/5 focus:bg-card focus:border-primary/50 transition-all duration-200 h-9 font-medium";
    const ghostTextareaClass =
        "bg-transparent border border-transparent shadow-none hover:bg-primary/5 focus:bg-card focus:border-primary focus:ring-1 focus:ring-primary transition-all duration-200 resize-none font-medium py-1.5 px-3 rounded-md";
    ...
    return (
        <div className="space-y-8 pb-20">
            <DashboardHeader
                ...
                description={
                    initialQuote
                        ? `Convirtiendo partes de COT-${initialQuote.quote_number}`
                        : "Generar cotización, códigos y partidas automáticamente."
                }
            ...
            {initialQuote && (
                <div className="mb-8 flex items-start gap-4 rounded-lg border border-green-500/30 bg-green-500/10 p-4">
                    <FolderPlus className="mt-0.5 h-6 w-6 flex-none text-green-500" />
                    <div>
                        <h3 className="text-sm font-bold uppercase text-green-600">
                            Convirtiendo COT-{initialQuote.quote_number}
                        </h3>
                        <p className="mt-1 text-xs text-muted-foreground">
                            Ajusta los materiales, genera subpartidas de ser necesario y añade cualquier archivo
                            pendiente. Al darle en "Guardar y Confirmar", esta cotización quedará marcada como aprobada
                            automáticamente.
                        </p>
                    </div>
                </div>
            ...
            <motion.div initial="hidden" animate="visible" variants={fadeIn}>
                <Card className="relative overflow-visible rounded-2xl border-border/40 bg-card/80 shadow-xl shadow-primary/5 backdrop-blur-sm">
                    <div className="absolute left-0 top-0 h-1 w-full rounded-t-2xl bg-gradient-to-r from-red-600 via-orange-500 to-red-600" />
                    <CardContent className="p-8">
                        <div className="relative">
                            {/* Ghost highlight for Tour Step 1 - First 3 fields */}
                            <div
                                id="project-header-first-three"
                                className="pointer-events-none absolute z-0 rounded-xl"
                                style={{
                                    top: "-8px",
                                    left: "-8px",
                                    width: "calc(100% + 16px)",
                            ...
                            <div className="mb-8 grid grid-cols-1 gap-x-8 gap-y-8 md:grid-cols-2 lg:grid-cols-4">
                                {/* Client */}
                                <div className="z-10 space-y-2.5" id="project-client-wrapper">
                                ...
                                {/* User / Requestor */}
                                <div className="z-10 space-y-2.5" id="project-user-wrapper">
                                    <Label className="text-xs font-medium uppercase tracking-wider text-muted-foreground">
                                        Usuario / Solicitante
                                    </Label>
                                    <SearchableSelect
                                        ...
                                        placeholder={
                                            selectedClient ? "Seleccionar o crear..." : "Selecciona Cliente primero"
                                        }
                                        className="w-full"
                        ...
                        <div
                            className="mb-8 grid grid-cols-1 gap-6 md:col-span-4 md:grid-cols-4"
                        ...
                        >
                            ...
                            <div className="space-y-2.5 md:col-span-1" id="project-items-count">
                                ...
                                <Input
                                    ...
                                    className={cn(
                                        "border-zinc-500/20 bg-background focus:ring-red-500/20",
                            ...
                            {((stagingFiles.length > 0 && !showPreview) || generatedItems.some((i) => i.isDemo)) && (
                                ...
                                >
                                    <Dropzone
                                        onFilesSelected={onFilesSelected}
                                        isUploading={isUploadingFiles}
                                        className="rounded-3xl border border-dashed border-zinc-500/20 bg-muted/10 p-6"
                                    >
                                        <div className="mb-4 flex items-center justify-between px-2">
                                            <h3 className="flex items-center gap-2 text-xl font-bold text-foreground">
                                                ...
                                                <span className="ml-2 text-sm font-normal text-muted-foreground">
                                                    ({stagingFiles.filter((f) => f.selected).length} archivos cargados)
                                                </span>
                                            </h3>
                                        ...
                                        <div className="custom-scrollbar grid max-h-[400px] grid-cols-2 gap-x-4 gap-y-8 overflow-y-auto p-5 md:grid-cols-4 lg:grid-cols-6">
                                            {stagingFiles.map((file) => (
                                                <div
                                                    key={file.id}
                                                    className={cn(
                                                        "group relative aspect-[3/4] overflow-hidden rounded-2xl border border-zinc-500/10 bg-muted/40 shadow-md backdrop-blur-sm transition-all duration-300",
                                                        file.selected
                                                            ? "scale-[1.02] bg-muted/60 ring-2 ring-zinc-500/30"
                                                            : "hover:scale-[1.02] hover:border-zinc-500/30"
                                                    )}
                                                >
                                                    <div className="absolute right-3 top-3 z-10">
                                                        <Button
                                                            ...
                                                            className="h-8 w-8 rounded-full bg-black/20 text-white opacity-0 backdrop-blur-md transition-opacity hover:bg-red-500 group-hover:opacity-100"
                                                        >
                                                            <X className="h-4 w-4" />
                                                        </Button>
                                                    </div>

                                                    <div className="flex h-full w-full items-center justify-center p-6 pb-12">
                                                        {file.thumbnail ? (
                                                            <img
                                                                src={file.thumbnail}
                                                                alt={file.name}
                                                                className="h-full w-full rounded-sm object-contain drop-shadow-lg"
                                                                referrerPolicy="no-referrer"
                                                            />
                                                        ) : (
                                                            <div className="flex flex-col items-center gap-2">
                                                                <FileText className="h-12 w-12 text-zinc-500 opacity-50" />
                                                    ...
                                                    <div className="absolute bottom-0 left-0 w-full truncate border-t border-zinc-500/5 bg-background/80 p-2.5 text-center text-[10px] font-medium uppercase tracking-tighter text-muted-foreground backdrop-blur-md">
                                                        {file.name}
                                                    </div>
                                                </div>
                                            ))}
                        ...
                        <div className="mt-10 flex justify-end">
                            ...
                            >
                                ...
                                {stagingFiles.length > 0
                                    ? `Generar Partidas desde Archivos (${stagingFiles.length})`
                    ...
                    >
                        <div className="flex items-center justify-between px-2">
                            <h3 className="flex items-center gap-3 text-2xl font-bold text-foreground">
                                ...
                                <span className="rounded-md bg-muted px-3 py-1 font-mono text-lg text-muted-foreground">
                                    {projectCode}
                                </span>
                            </h3>
                        </div>
                        ...
                        <Card
                            className="overflow-hidden rounded-2xl border-border/40 bg-card shadow-2xl"
                            id="project-preview-table"
                        >
                            <div className="overflow-x-auto">
                                <SharedItemsTable
                                    ...
                                    onReorder={(newItems) =>
                                        setGeneratedItems(resequenceItems(newItems as any, projectCode || "PRJ"))
                                    }
